...
def get_stock_data(stock_code, start_date, end_date):
    try:
        # 格式化股票代码，确保满足akshare的要求
        # A股股票代码处理（上海股票以'6'开头，深圳股票以'0'或'3'开头）
        ...
        st.info(f"正在获取 {formatted_code} 的数据...")
        
        # 尝试使用格式化的代码获取数据
        try:
            df = ak.stock_zh_a_hist(symbol=formatted_code, period="daily", start_date=start_date, end_date=end_date, adjust="qfq")
        except Exception as e1:
            st.warning(f"尝试格式1失败: {e1}，尝试原始代码...")
            # 如果格式化代码失败，尝试使用原始代码
        ...
        if not df.empty:
            # 检查必要的列是否存在
            expected_columns = ['日期', '开盘', '收盘', '最高', '最低', '成交量', '成交额', '振幅', '涨跌幅', '换手率']
            ...
            if set(expected_columns) != set(actual_columns):
                ...
                # 可能的列名映射
                possible_mappings = {
                    '日期': ['date', 'time', 'datetime', '时间'],
                    '开盘': ['open', 'open_price', '开盘价'],
                    '收盘': ['close', 'close_price', '收盘价'],
                    '最高': ['high', 'high_price', '最高价'],
                    '最低': ['low', 'low_price', '最低价'],
                    '成交量': ['volume', 'vol', '成交量'],
                    '成交额': ['amount', 'turnover', '成交额'],
                    '振幅': ['amplitude', 'amp', '振幅'],
                    '涨跌幅': ['change_percent', 'pct_change', 'pct_chg', '涨跌幅', '涨跌百分比'],
                    '换手率': ['turnover_rate', 'turnover_ratio', '换手率']
                }
                
                # 尝试匹配列名
                for expected, possibles in possible_mappings.items():
                    for actual in actual_columns:
                        if actual.lower() in possibles or actual == expected:
            ...
            if '日期' in df.columns and not pd.api.types.is_string_dtype(df['日期']):
                df['日期'] = df['日期'].astype(str)
                
            return df
        else:
            st.error("获取的数据为空，请尝试其他股票代码或检查网络连接")
            return pd.DataFrame()
            
    except Exception as e:
        st.error(f"获取股票数据失败: {e}")
        try:
            # 第三种尝试：使用akshare的另一个API
            st.warning("尝试使用备用API获取数据...")
            if stock_code.startswith('6'):
                new_code = f"sh{stock_code}"
            else:
                new_code = f"sz{stock_code}"
                
            df = ak.stock_zh_a_daily(symbol=new_code, start_date=start_date, end_date=end_date, adjust="qfq")
...
def get_stock_fundamental(stock_code):
    try:
        # 格式化股票代码，与get_stock_data函数相同的逻辑
        ...
        try:
            ...
            st.warning(f"获取财务指标失败: {e1}，尝试其他方法...")
            latest_financial = pd.Series()
        ...
        try:
            ...
            st.warning(f"获取市场指标失败: {e2}，尝试备用方法...")
            # 尝试使用备用方法获取基本面数据
            try:
                # 可能的备用方法
        ...
        result = {
            "股票代码": original_code,
            "市盈率(动态)": stock_info.loc[stock_info['指标名称'] == '市盈率(动态)', '最新值'].values[0] if not stock_info.empty and '指标名称' in stock_info.columns and '市盈率(动态)' in stock_info['指标名称'].values else None,
            "市净率": stock_info.loc[stock_info['指标名称'] == '市净率', '最新值'].values[0] if not stock_info.empty and '指标名称' in stock_info.columns and '市净率' in stock_info['指标名称'].values else None,
...
def filter_stocks(pe_min, pe_max, pb_min, pb_max, roe_min, growth_min, max_stocks=200):
    ...
    for i, (_, row) in enumerate(stock_list.iterrows()):
        ...
        try:
            ...
            if not stock_code.isdigit() and len(stock_code) > 6:
                ...
                if len(stock_code) > 6:  # 如果还是太长，截取前6位
                    stock_code = stock_code[:6]
                elif len(stock_code) < 6:  # 如果太短，可能格式不对
                    continue
            
            # 获取基本面数据，带有重试机制
            retry = 0
            fund_data = pd.DataFrame()
            while retry < max_retries and fund_data.empty:
                try:
                    ...
                    if retry >= max_retries:
                        st.warning(f"获取 {stock_code} 数据失败，跳过")
            ...
            if valid_data:
                try:
                    ...
                    # 应用筛选条件
                    pe_condition = (pe_min <= pe <= pe_max) if pe_min != 0 or pe_max != 0 else True
                    pb_condition = (pb_min <= pb <= pb_max) if pb_min != 0 or pb_max != 0 else True
                    
                    if pe_condition and pb_condition and roe >= roe_min and growth >= growth_min:
            ...
            error_count += 1
            # 如果错误太多，提前终止
            if error_count > min(20, total_stocks * 0.2):  # 限制错误率不超过20%
                st.warning(f"遇到过多错误 ({error_count})，提前终止筛选")
                break
            continue
    
    progress_bar.progress(1.0)
    status_text.text(f"筛选完成，共找到 {len(filtered_stocks)} 只符合条件的股票 (错误: {error_count}, 重试: {retry_count})")
    ...
    # 确保数值列格式正确
    numeric_cols = ["市盈率", "市净率", "ROE(%)", "营收增长率(%)", "净利润增长率(%)"]
